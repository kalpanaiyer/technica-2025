...
interface Task {
  ...
  completed: boolean;
  tokens: number;
  awarded?: boolean; // whether notes have been awarded for completing this task
}
    ...
    const unsub = onAuthStateChanged(auth, (user) => {
      if (user?.uid) {
        getUserNotes(user.uid)
          .then((total) => setNotesTotal(total))
          .catch((err) => console.error('Error fetching notes total:', err));
      } else {
        setNotesTotal(null);
      }
    });
  ...
  const handleToggleComplete = (id: number) => {
    setTasks((prev) =>
      prev.map((task) => {
        ...
        // If marking as complete and not yet awarded, award notes
        if (willComplete && !task.awarded) {
          const user = auth.currentUser;
          if (user?.uid) {
            // update notes in backend and update local notesTotal when done
            addNotesToUser(user.uid, task.tokens)
              .then((newTotal) => {
                setNotesTotal(newTotal);
              })
              .catch((err) => console.error('Error awarding notes:', err));

            return { ...task, completed: true, awarded: true };
          } else {
            // not signed in — don't award, but still toggle completion locally
            console.warn('User not signed in; cannot award notes');
            return { ...task, completed: true };
          }
        }

        // If toggling back to incomplete, keep awarded flag as-is (don't remove notes)
      ...
      {!isExpanded ? (
        ...
        <div className={styles.expanded_panel}>
          <div className={styles.header}>
            <h2>Todo</h2>
            <span className={styles.arrow_back} onClick={toggleExpanded}>
              ‹
            </span>
          </div>
          ...
          <div className={styles.task_list}>
            {tasks.map((task) => (
              <div key={task.id} className={styles.task_item}>
                ...
                <span
                  className={
                    task.completed ? styles.task_completed : styles.task_text
                  }
                >
                  {task.text}
                </span>
          ...
          {!showInput ? (
            ...
            <div className={styles.input_section}>
              <input
                ...
                autoFocus
              />
              <button onClick={handleAddTask} className={styles.add_btn}>
                Add
              </button>
            </div>
          )}
