...
      setError("Please set your ELEVENLABS_API_KEY to enable sound generation.");
      return;
    }
    ...
    if (audioRef.current) {
      if (isPlaying) {
        ...
        audioRef.current.play().catch(e => {
          ...
          setError("Playback failed. Please ensure your browser allows media playback without a user gesture.");
        });
      }
...
            disabled={isLoading}
            className="flex-grow px-4 py-3 border border-blue-300 rounded-lg focus:ring-blue-500 focus:border-blue-500 transition duration-150 bg-white shadow-sm"
          />
          <button 
...
              className="p-3 bg-[#478094] text-white rounded-full hover:bg-[#1C4857] transition duration-150 shadow-lg flex-shrink-0"
            >
              {isPlaying ? (
                <svg className="w-6 h-6" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect width="6" height="16" x="4" y="4"/><rect width="6" height="16" x="14" y="4"/></svg> 
              ) : (
                <svg className="w-6 h-6" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polygon points="5 3 19 12 5 21 5 3"/></svg>
              )}
            </button>
            
            <p className="font-semibold text-gray-700 text-sm">
              {isPlaying ? `Now Playing: "${prompt}"` : `Ready to Play: "${prompt}"`} 
            </p>
