...
const ALL_ENVIRONMENTS: Environment[] = [
    { id: 'underwater', name: 'Under The Sea', image: '/images/underwater.png' },
    { id: 'rainforest', name: 'Rainforest', image: '/images/rainforest_bg.jpg' },
    { id: 'cafe', name: 'Cafe', image: '/images/cafe_bg.gif' },
];
...
const Dashboard: React.FC = () => {
    ...
    const loadAvailableEnvironments = async () => {
        ...
        if (!userId) {
            // If no user, just show default environment
            setAvailableEnvironments([ALL_ENVIRONMENTS[0]]);
            setLoading(false);
            return;
        }
        ...
        try {
            ...
            // Always include "Under The Sea" as default
            const available = [ALL_ENVIRONMENTS[0]];
            
            // Add purchased environments
            if (purchases.includes('Rainforest')) {
                available.push(ALL_ENVIRONMENTS[1]);
            }
            if (purchases.includes('Cafe')) {
                available.push(ALL_ENVIRONMENTS[2]);
            }
    ...
    if (canStartSession) {
        // Determine which session page to navigate to based on environment
        let sessionPath = '/session'; // default underwater
        
        if (selectedEnviroment === 'rainforest') {
            sessionPath = '/rainforestsession';
        } else if (selectedEnviroment === 'cafe') {
            sessionPath = '/cafesession';
        }
    ...
    {availableEnvironments.map((env) => (
        <div 
            key={env.id}
            onClick={() => setSelectEnviroment(selectedEnviroment === env.id ? null : env.id)}
            className={selectedEnviroment === env.id ? styles.selected : ''}
            style={{
        ...
        >
            ...
            <p className='font-[Pixelify_Sans] text-white text-center' style={{
    ...
    {/* "Head to store" card - only show if user doesn't have all environments */}
    {availableEnvironments.length < ALL_ENVIRONMENTS.length && (
